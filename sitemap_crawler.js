...
const argv = yargs(hideBin(process.argv))
  .option('sitemap', {
    alias: 's',
    description: 'Path to sitemap XML file',
    type: 'string',
  ...
  .option('start', {
    description: 'Start from collection index (0-based)',
    type: 'number',
    default: 0
  })
  .option('limit', {
    alias: 'l',
    description: 'Maximum collections to crawl (0 for all)',
    type: 'number',
    default: 0
  })
  .option('products-per-collection', {
    alias: 'p',
    description: 'Maximum products to crawl per collection (0 for all)',
    type: 'number',
    default: 0
  })
  .option('timeout', {
...
/**
 * Parse sitemap XML to extract collection URLs
 */
async function parseCollectionsFromSitemap(sitemapPath) {
  try {
    console.log(`Parsing sitemap file: ${sitemapPath}`);
    const sitemapXml = await fs.readFile(sitemapPath, 'utf-8');
    const parsed = await parseStringPromise(sitemapXml, { explicitArray: false });
    ...
    // Extract collection URLs and titles
    const collections = urls.map(item => {
      let title = 'Unknown Collection';
      
      // Try to extract title from image data if available
      if (item.image && item.image.title) {
        title = item.image.title;
      } else {
        // Extract name from URL as fallback
        const urlParts = item.loc.split('/');
...
/**
 * Main function to crawl Shopify products from sitemap collections
 */
async function crawlShopifyProductsFromSitemap() {
  ...
  // Try to load existing progress file if it exists
  try {
    if (await fs.pathExists(argv.output)) {
      console.log(`Found existing progress file: ${argv.output}, loading...`);
      const existingData = await fs.readJSON(argv.output);
      if (existingData.products && Array.isArray(existingData.products)) {
        result = existingData;
        console.log(`Loaded ${result.products.length} existing products from progress file`);
      }
    }
  } catch (error) {
  ...
  // Create a map of already crawled products
  const crawledProducts = new Map();
  result.products.forEach(product => {
    if (product.handle) {
      crawledProducts.set(product.handle, true);
    }
  });
  
  console.log(`\n=== STEP 1: PARSING SITEMAP ===`);
  // Get collection URLs from sitemap
  const collections = await parseCollectionsFromSitemap(argv.sitemap);
  ...
  // Update result collections list
  ...
  try {
    ...
    const startIndex = Math.max(0, Math.min(argv.start, collections.length - 1));
    const endIndex = argv.limit > 0 ? 
                    Math.min(startIndex + argv.limit, collections.length) : 
                    collections.length;
    
    // Process collections
    for (let colIndex = startIndex; colIndex < endIndex; colIndex++) {
      const collection = collections[colIndex];
      console.log(`\nCollection ${colIndex + 1}/${collections.length}: "${collection.title}" (${collection.url})`);
      
      // Check if collection was already crawled
      const collectionIndex = result.collections.findIndex(c => c.url === collection.url);
      if (collectionIndex !== -1 && result.collections[collectionIndex].crawled) {
        console.log(`Collection "${collection.title}" already crawled, skipping...`);
        continue;
      }
      ...
      try {
        ...
        // Track products found in this collection
        let collectionProductCount = 0;
        let totalCollectionProducts = 0;
        
        // Crawl each page in this collection
        for (let currentPage = 1; currentPage <= totalPages; currentPage++) {
          console.log(`Processing page ${currentPage}/${totalPages} of collection "${collection.title}"`);
          ...
          totalCollectionProducts += uniqueProductLinks.length;
          
          // Check if we should limit products per collection
          const productsPerCollection = argv['products-per-collection'];
          if (productsPerCollection > 0 && collectionProductCount >= productsPerCollection) {
            console.log(`Reached product limit (${productsPerCollection}) for this collection, moving to next collection`);
            break;
          }
          ...
          for (const [prodIndex, productUrl] of uniqueProductLinks.entries()) {
            try {
              ...
              // Check if we've reached the product limit for this collection
              if (productsPerCollection > 0 && collectionProductCount >= productsPerCollection) {
                console.log(`Reached product limit for this collection, moving to next product`);
                break;
              }
              ...
              const productData = await page.evaluate(() => {
                ...
                try {
                  ...
                  // Ensure no variant has a null image - do a final check
                  variants.forEach(variant => {
        ...
        // Mark collection as crawled and update product count
        if (collectionIndex !== -1) {
          result.collections[collectionIndex].productCount = collectionProductCount;
          result.collections[collectionIndex].totalProducts = totalCollectionProducts;
          result.collections[collectionIndex].crawled = true;
        }
        ...
        // Mark collection as failed if present in result collections
        if (collectionIndex !== -1) {
          result.collections[collectionIndex].error = error.message;
        }
      }
    }
    
    console.log(`\n=== STEP 3: CRAWLING COMPLETED ===`);
    console.log(`Total collections crawled: ${result.collections.filter(c => c.crawled).length}/${result.collections.length}`);
...
crawlShopifyProductsFromSitemap().catch(err => {
